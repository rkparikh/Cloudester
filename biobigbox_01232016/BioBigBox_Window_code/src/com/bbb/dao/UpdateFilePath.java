...
public class UpdateFilePath {
    ...
    /**
     * updateFileName(FileBean fileBean) METHOD USE FOR UPDATE FILE Name
     */
    public static boolean updateFileName(FileBean fileBean) {
        boolean statu = false;
        PreparedStatement ps = null;
        try {
            String Update_Query = "UPDATE FILE SET FULL_FILE_PATH_LOCAL=? WHERE FILE_ID=?";
            Connection conn = Initialize.getConn();
            logger.info("Update File Name   : ");
            ps = conn.prepareStatement(Update_Query);
            ps.setString(1, fileBean.getFull_file_path());
            ps.setInt(2, fileBean.getId());
            ps.executeUpdate();
            statu = true;
            logger.info("UPDATE FILE SET FULL_FILE_PATH_LOCAL=? WHERE FILE_ID=?'");
    ...
    public static void updateFileStatus(String fileIds) {
        ...
        try {
            if (!fileIds.trim().equals("")) {
                String Update_Query = "UPDATE FILE SET STATUS=1 WHERE BBB_ID in (" + fileIds + ") AND FULL_FILE_PATH_LOCAL like '" + Initialize.getDefaultDirectory().replaceAll("'","''") + "%'";
    ...
    /**
     * updateFileForCancelStatus() METHOD USE FOR UPDATE FILE STATUS
     */
    public static boolean updateFileForCancelStatus() {
        boolean status = false;
        PreparedStatement ps = null;
        try {
            String Update_Query = "UPDATE FILE SET  STATUS=5 WHERE FULL_FILE_PATH_LOCAL like '" + Initialize.getDefaultDirectory().replaceAll("'","''") + "%'";
            Connection conn = Initialize.getConn();
            logger.info("Update File STATUS   : ");
            ps = conn.prepareStatement(Update_Query);
            ps.executeUpdate();
     ...
     /**
     * updateFolderSyncRequest() METHOD USE FOR UPDATE FOLDER SYNC REQUEST STATUS
     */
    public static boolean updateFolderSyncRequest() {
        boolean status = false;
        PreparedStatement ps = null;
        try {
            String Update_Query = "UPDATE FILE SET  FOLDER_SYNC_STATUS=0 WHERE FOLDER_SYNC_STATUS=1";
            Connection conn = Initialize.getConn();
            logger.info("Update updateFolderSyncRequest STATUS   : ");
            ps = conn.prepareStatement(Update_Query);
            ps.executeUpdate();
            status = true;
            ...
            sqlExcept.printStackTrace();
            logger.error("Update  updateFolderSyncRequest Status Error " + sqlExcept);
        }
        return status;
    }
 /**
     * updateFolderSyncRequestWithId() METHOD USE FOR UPDATE FOLDER SYNC REQUEST STATUS
     */
    public static boolean updateFolderSyncRequestWithId(String folder_id) {
        boolean status = false;
        PreparedStatement ps = null;
        try {
            String Update_Query = "UPDATE FILE SET  FOLDER_SYNC_STATUS=0 WHERE FOLDER_SYNC_STATUS AND FOLDER_ID='"+folder_id+"'";
            Connection conn = Initialize.getConn();
            logger.info("Update updateFolderSyncRequestWithId   : ");
            ps = conn.prepareStatement(Update_Query);
            ps.executeUpdate();
            status = true;
            ...
            sqlExcept.printStackTrace();
            logger.error("Update  updateFolderSyncRequestWithId  Error " + sqlExcept);
        }
        return status;
    }
    /**
