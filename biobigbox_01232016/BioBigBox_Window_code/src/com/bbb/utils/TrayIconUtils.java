...
public class TrayIconUtils {
    ...
    public static ImageIcon syncingStop = new ImageIcon(TrayIconUtils.class.getResource("/syncingStop.png"));
    public static ImageIcon syncigOn = new ImageIcon(TrayIconUtils.class.getResource("/syncingOn.png"));
        ...
        switch (mode) {
            ...
            case "Sync": {
                ...
                if (SystemTray.isSupported()) {
                    ...
                    popup.add(item);
                    popup.addSeparator();
                    item = new MenuItem("Restore Data");
                    item.addActionListener(new TrayIconUtils.ShowMessageListener(trayIcon,
            ...
            switch (message) {
                case "Sync": {
                    try {
                        if (Initialize.Uptodatemsg) {
                            ...
                            System.out.println("in sync start");
                ...
                case "Log": {
                    ...
                    try {
                        ...
                        //File f = new File(System.getProperty("user.home") + File.separator + "BBBLog/log.txt");

                        // Desktop.getDesktop().open(new File(System.getProperty("user.home") + File.separator + "logfile.txt"));
                    } catch (Exception ex) {
...
//                    WebServices.updateFolderSync(Initialize.is_folder_sync);
                ...
                case "restore": {
                    try {
                        Desktop.getDesktop().browse(new URL("http://biobigbox.com").toURI());
                        logger.info("Account");
                    } catch (Exception ex) {
                        logger.error("Account  :" + ex);
                    }
