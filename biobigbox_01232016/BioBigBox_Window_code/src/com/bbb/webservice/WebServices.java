...
//            DateFormat timeFormatter = new SimpleDateFormat("hh:mma");
    ...
    public static FileBean getUploadFileID(String url, FileBean filebean) {
        ...
        try {
            ...
            //String uploadJson = "{\"token\":\"" + Initialize.getLoginToken() + "\",\"size\":\"" + BBBUtils.getFileSize(url) + "\",\"type\":\"" + BBBUtils.getContentType(filePath[filePath.length - 1]) + "\",\"name\":\"" + filePath[filePath.length - 1] + "\",\"encoded_name\":\"" + filePath[filePath.length - 1] + "\",\"file_id\":" + filebean.getBbb_id_inString() + ",\"folder_path\":\"" + directory + "/" + folder_path + "\"}";
            ...
            if (response != null && response.getStatusLine().getStatusCode() == 200) {
                ...
                while ((output = br.readLine()) != null) {
                    ...
                    if ((boolean) jsonObject.get("success")) {
                        ...
                        System.out.println("get biobigbox version :"+innerJsonObject.get("version").toString());
                                ...
                                new Thread() {
                                    public void run() {
                                        try {
                                            ...
                                            FileOutputStream outputStream = new FileOutputStream(System.getProperty("user.home") + File.separator + "BBBApp.exe");
                                            ...
                                            ProcessBuilder processBuilder = new ProcessBuilder(System.getProperty("user.home") + File.separator + "BBBApp.exe");
                                            Process process = processBuilder.start();                                            
                                            if(process.waitFor()==0){
                                                JOptionPane.showMessageDialog(null, "BioBigBox desktop sync has been upgraded successfully.", "BBB Updation", JOptionPane.INFORMATION_MESSAGE, appImageIcon);
                                            }
...
//                                theFrame.setVisible(true);
    ...
    public static boolean getUpdatedVersion() {
        ...
        String getVersionNo[] = version.split(" ");
        System.out.println("get version no:" + getVersionNo[1]);
        //float versionNo = Float.parseFloat(getVersionNo[1]);
        try {
            HttpClient httpClient = new DefaultHttpClient();
            HttpPost postRequest = new HttpPost(Initialize.checkUpdatedVersion);
            StringBuilder sb = new StringBuilder("{\"token\":\"" + Initialize.getLoginToken() + "\"}");
